...
TV_LIBRARIES = parser.get('plex', 'library').split(',')
...
def script():
    ...
    all_tvshows = []
    for tv_lib in TV_LIBRARIES:
        try:
            print("Retrieving a list of TV Shows from the '{library}' library in Plex.".format(library=tv_lib))
            print("\n")
            tv_library = plex.library.section(tv_lib)
            library_language = tv_library.language
            all_tvshows.extend(tv_library.all())
        except:
            print("The '{library}' library does not exist in Plex.".format(library=tv_lib))
            print("Please check that config.ini exists, and is correct.")
            print("\n")
            input("Press Enter to exit")
            sys.exit()
    print("Finding TV Shows in libraries...")
    print("\n")
    ...
    with open(CSV_LOCATION) as csv_file:
        ...
        for row in csv_reader:
            ...
            if bool(row['year']) == False:
                ...
                for tv in all_tvshows:
                    if (tv.title.lower() == title.lower() and str(tv.year) == year):
                        add_collection(tv.librarySectionID, tv.ratingKey, tv.title, tv.year)
    sys.exit()
script()
